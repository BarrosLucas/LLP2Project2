...
bool SistemaImobiliaria::cadastraImovel(Imovel imovel){
	if(imovel.getEndereco().getBairro().length()>0 && imovel.getEndereco().getCep().length()==9 && imovel.getEndereco().getCidade().length()>0 && imovel.getEndereco().getLogradouro().length()>0 && imovel.getEndereco().getNumero()>0){
...
std::vector<std::string> SistemaImobiliaria::getDescricaoImoveis(){
	...
	for(Imovel *imovel : imoveis){
		if(instanceof->instanceof(imovel)==CASA){
			Casa *casa = dynamic_cast<Casa*>(imovel);
			retorno.push_back(casa->getDescricao());
		}else if(instanceof->instanceof(imovel)==APARTAMENTO){
			Apartamento *apartamento = dynamic_cast<Apartamento*>(imovel);
			retorno.push_back(apartamento->getDescricao());
		}else if(instanceof->instanceof(imovel)==TERRENO){
			Terreno *terreno = dynamic_cast<Terreno*>(imovel);
			retorno.push_back(terreno->getDescricao());
		}
	}
...
std::vector<Imovel*> SistemaImobiliaria::getImoveisParaAlugarPorBairro(std::string bairro){
	...
	for(Imovel *imovel: imoveis){
		if(manipulador->stringsCombinam(bairro,imovel->getEndereco().getBairro()) && imovel->getTipoOferta()==IMOVEL_PARA_ALUGAR){
			retorno.push_back(imovel);
		}
	}
...
std::vector<Imovel*> SistemaImobiliaria::getImoveisParaVenderPorBairro(std::string bairro){
	std::vector<Imovel*> retorno;

	ManipuladorString *manipulador = new ManipuladorString();

	for(Imovel *imovel: imoveis){
		if(manipulador->stringsCombinam(bairro,imovel->getEndereco().getBairro()) && imovel->getTipoOferta()==IMOVEL_PARA_VENDER){
			retorno.push_back(imovel);
		}
	}
...
std::vector<Imovel*> SistemaImobiliaria::getImoveisPorCidade(std::string cidade){
	...
	for(Imovel *imovel: imoveis){
		if(manipulador->stringsCombinam(cidade,imovel->getEndereco().getCidade())){
			retorno.push_back(imovel);
		}
	}
